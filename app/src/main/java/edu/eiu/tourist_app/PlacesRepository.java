...
public class PlacesRepository {
    ...
    private WikipediaService wikipediaSevice;

    public PlacesRepository(WikipediaService wikipediaSevice){
        this.wikipediaSevice=wikipediaSevice;
    }
    ...
    public LiveData<List<WikipediaPage>> getTouristSites() {
        ...
        wikipediaSevice.getPlaces().enqueue(new Callback<WikipediaResponse>() {
            @Override
            ...
            public void onFailure(Call<WikipediaResponse> call, Throwable t) {
                Log.e("PlaceRepository","Wikipedia request failed",t);
            }
        });
...
//        task.execute();
    ...
    private InputStream doRequest() throws IOException {
        Retrofit retrofit = new Retrofit.Builder()
                ...
                .baseUrl("https://en.wikipedia.org").build();

        wikipediaSevice= retrofit.create(WikipediaService.class);
        wikipediaSevice.getPlaces().enqueue(new Callback<WikipediaResponse>() {
            @Override
            public void onResponse(Call<WikipediaResponse> call, Response<WikipediaResponse> response) {
    ...
    private List<WikipediaPage> handleResponse(InputStream response) {
        Gson gson = new Gson();
        BufferedReader reader = new BufferedReader(new InputStreamReader(response));// transfer json data to object wiki
        ...
        List<WikipediaPage> pages = new ArrayList<WikipediaPage>(pageMap.values());
        return pages;
    }
}
